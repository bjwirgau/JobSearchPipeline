...
resume_generator_schedule="${JOB_AGENT_RESUME_GENERATOR_CRON_SCHEDULE:-* * * * *}"
...
awk \
    ...
    '
        ...
        index($0, "/scripts/run_resume_generator.sh") { next }
        { print }
    ' \
...

{
    ...
    printf 'JOB_AGENT_RESUME_GENERATION_BATCH_LIMIT=%s\n' "$resume_limit"
    printf 'JOB_AGENT_RESUME_GENERATION_BATCH_FORMAT=%s\n' "$resume_format"
    ...
    printf '%s %s/scripts/run_resume_generator.sh\n' \
        "$resume_generator_schedule" "$project_root"
    printf '%s\n' "$end_marker"
} >>"$next_crontab"
