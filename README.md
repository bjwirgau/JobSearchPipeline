...
`match`, `title`, `company`, `location`, `salary`, `source`, and `url` columns.
The match is nullable during search and updated to a score between `0` and `1`
after the scoring stage. Candidate identity and preferences remain in
`data/candidate_profile.json`; application records are not persisted in the
current schema.
